...
HY2_URI="hysteria2://${PASSWORD}@${SERVER_IP}:443/?sni=cloudflare.com&pinSHA256=${CERT_PIN}#Magicat_Node"
...
echo "----------------"
echo "# PC端配置"
printf '{"serverip":"%s","password":"%s","keysha256":"%s"}\n' "$SERVER_IP" "$PASSWORD" "$KEY_SHA256"
echo "----------------"
echo "# v2rayN/v2rayNG"
echo "${HY2_URI}"
echo "----------------"