...
# bash /root/add_user.sh YYYY-MM-DD YYYY-MM-DD
...
curl -fsSL --proto '=https' --proto-redir '=https' --tlsv1.2 -o "$_clean_tmp" "$CLEAN_URL" || { echo "清理脚本下载失败"; rm -f "$_clean_tmp"; exit 1; }
...
for i in "${!NAMES[@]}"; do
  ...
  UUID="${UUIDS[$i]}"
  PASSWORD="${PWS[$i]}"
  HY2_URI="hysteria2://${PASSWORD}@${SERVER_IP}:${PORT}/?pinSHA256=${CERT_PIN}#${N}_HY2"
